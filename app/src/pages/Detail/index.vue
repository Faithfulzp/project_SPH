            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                <a
                  href="javascript:"
                  class="mins"
                  @click="skuNum <= 0 ? (skuNum = 0) : skuNum--"
                  >-</a
                >
              </div>
              <div class="add">
...
export default {
  ...
  methods: {
    // 改变高亮，接收点击的值和值所在的整个数组
    checkActive(spuSaleAttrValue, arr) {
      // 排他思想
      // 将整个数组里的isChecked值给清0
      arr.forEach((item) => {
        item.isChecked = 0;
      });
      // 给点击的那个值，赋isChecked为1 高亮
    ...
    async addshopcar() {
      // 数据要求是对象的形式
      // 1、发请求--将产品加入数据库，
      // dispatch调用函数的返回值是一个promise对象，
      // 通过promise对象的返回值判断成功与失败就不需要在仓库中存储成功与失败的数据了
      try {
        let result = await this.$store.dispatch("detail/addOrUpdateCart", {
          skuId: this.$route.params.skuid,
          skuNum: this.skuNum,
        });
        // 将商品信息存储在 会话存储 中，会话存储：浏览器关掉表示会话结束，会话存储的数据则销毁
        // 会话存储不能直接存储对象，需要先转化成字符串
        sessionStorage.setItem("SKUINFO", JSON.stringify(this.skuInfo));
        // 2、服务器存储成功--进行路由跳转传递参数，将整个skuInfo对象通过路由传递的话路由路径很长，
        this.$router.push({
          name: "addcartsuccess",
          query: {
            skuNum: this.skuNum,
